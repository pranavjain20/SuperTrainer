...
## Phase 2a Day 7 — Dual Data Formats in JSONB

- **When two producers write different keys to the same JSONB field, every consumer must handle BOTH — and priority order matters.** Seed data writes `weight_kg: 60`. Voice pipeline writes `weight: 60`. The display function (`getSetWeight`) read `weight_kg` first. The edit modal saved as `weight` (voice format) but preserved `weight_kg` via object spread. Result: after editing weight from 60→90, the card still showed 60 because it read the stale `weight_kg` from the spread. Fix was two-part: (1) display reads `weight` before `weight_kg`, (2) edit modal strips `weight_kg` on save to normalize to voice format. The rule: when JSONB has no enforced schema and multiple producers exist, the EDITOR must normalize to one format, and the DISPLAY must read the normalized format first.

- **`invalidateQueries` triggers async background refetches — use `setQueryData` for instant UI updates.** TanStack Query's `invalidateQueries` marks queries stale and refetches, but the UI doesn't update until the refetch completes. For inline edits where the mutation response already has the new data, `setQueryData` patches the cache synchronously — cards update instantly with zero flicker.
